...
function createTest (testName, html, expected) {
  test('normalize(parse(elm).nodes) ' + testName, function (t) {
    var elm = document.createElement('div');
    elm.contentEditable = true;
    elm.innerHTML = html.trim();

    t.deepEqual(normalize(parse(elm).nodes), expected);

    t.end();
  });
}

test('normalize(parse(elm).nodes) whitespace \t', function (t) {
  var elm = document.body.appendChild(document.createElement('p'));
  elm.appendChild(document.createTextNode('\tbeep\tboop\t'));
  t.deepEqual(normalize(parse(elm).nodes), [{
    type: 'paragraph',
    children: [{
      type: 'text',
      content: ' beep boop ',
...
test('normalize(parse(elm).nodes) Multiple text nodes', function (t) {
...
test('normalize(parse(elm).nodes) empty text node', function (t) {
  var elm = document.body.appendChild(document.createElement('div'));
